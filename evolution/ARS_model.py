...
#         
    ...
    def findwhichtarget(self, radius):
        # This method helps locate a target object based on its coordinates: x, y
        
        dis, indices = Grid.kdtree.query([self.x,self.y], k=1000, distance_upper_bound = radius)
...
#             if np.size(indices) > 1 :
                ...
                while k < size:
                    ...
                    if targets_list[i].food == 1 :
                        ...
                        return [targets_list[i].num,  targets_list[i].x, targets_list[i].y]
                    k = k + 1
                return None
            else:
                if targets_list[indices[0]].food == 1 :
                    targets_list[indices[0]].food = 0 
                    Grid.deleted_targets += 1
                    return [targets_list[indices[0]].num, targets_list[indices[0]].x, targets_list[indices[0]].y]
                else:
                    return None
...
class Agent:
    ...
    def update(self, agents_list,r):  ##check if on a target or not
        ...
        ##check if agent is on a  patch 
        out = Grid.findwhichtarget(self, minstep)
        ...
        ##if not
            ...
            self.tree = 0
            ###if currently not a patch, but found resource in last step 
            #####expand search radius then 
            if self.justfound == 1 :
                out = Grid.findwhichtarget(self, minstep*2)
...
#                     print(num, x, y)
    ...
    def eat_food(self, num):
        self.eff = self.food / self.tdis
        self.tree = num   ###signal to others
    ...
    def move(self, r):
        ...
        if self.tw_switch == 1 :
            ...
            if newtarget != None :  ###start new tw
                if self.d > 0 :
                    self.step()
                    if self.d <= 0 :
                        self.tw_switch = 0
            ...
            if newtarget != None :   ###start a tw
                ...
                self.d = rw_update[0]  ###distance to move
                self.theta = rw_update[1]  ###direction 
